...
const Header: React.FC = () => {
    ...
    return (
        <>
            <header className="fixed top-0 left-0 right-0 z-50 bg-[var(--color-bg)] flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[var(--color-border)] px-4 sm:px-6 md:px-8 lg:px-10 py-3">
                ...
                <div className="flex items-center gap-4 sm:gap-3 flex-1 justify-end">
                    ...
                    <nav id="site-nav" className="hidden lg:flex items-center gap-4 xl:gap-6">
                        ...
                        <a className="text-[var(--color-text)] text-sm font-medium leading-normal p-2 px-3 h-10 rounded-md hover:bg-[var(--color-bg-hover)]" href="#education" onClick={e => { e.preventDefault(); const el = document.getElementById('education'); if (el) { const y = el.getBoundingClientRect().top + window.scrollY - 64; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Education</a>
            ...
            <div className="h-16" />
            {/* Mobile dropdown nav */}
            <div className={`lg:hidden fixed top-16 left-0 right-0 z-40 border-b border-[var(--color-border)] ${mobileOpen ? 'block' : 'hidden'}`}>
                <nav className="flex flex-col gap-1 px-4 sm:px-6 py-2 bg-[var(--color-bg)]" aria-label="Mobile">
                    <a className="text-[var(--color-text)] py-2 px-3 text-sm font-medium leading-normal rounded-md bg-[var(--color-bg-hover)]" href="#about" onClick={e => { e.preventDefault(); setMobileOpen(false); const el = document.getElementById('about'); if (el) { const y = el.getBoundingClientRect().top + window.scrollY - 64; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>About</a>
                    <a className="text-[var(--color-text)] py-2 px-3 text-sm font-medium leading-normal rounded-md bg-[var(--color-bg-hover)]" href="#experience" onClick={e => { e.preventDefault(); setMobileOpen(false); const el = document.getElementById('experience'); if (el) { const y = el.getBoundingClientRect().top + window.scrollY - 64; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Experience</a>
                    <a className="text-[var(--color-text)] py-2 px-3 text-sm font-medium leading-normal rounded-md bg-[var(--color-bg-hover)]" href="#skills" onClick={e => { e.preventDefault(); setMobileOpen(false); const el = document.getElementById('skills'); if (el) { const y = el.getBoundingClientRect().top + window.scrollY - 64; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Skills</a>
                    <a className="text-[var(--color-text)] py-2 px-3 text-sm font-medium leading-normal rounded-md bg-[var(--color-bg-hover)]" href="#projects" onClick={e => { e.preventDefault(); setMobileOpen(false); const el = document.getElementById('projects'); if (el) { const y = el.getBoundingClientRect().top + window.scrollY - 64; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Projects</a>
                    <a className="text-[var(--color-text)] py-2 px-3 text-sm font-medium leading-normal rounded-md bg-[var(--color-bg-hover)]" href="#education" onClick={e => { e.preventDefault(); setMobileOpen(false); const el = document.getElementById('education'); if (el) { const y = el.getBoundingClientRect().top + window.scrollY - 64; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Education</a>
                    <a className="text-[var(--color-text)] py-2 px-3 text-sm font-medium leading-normal rounded-md bg-[var(--color-bg-hover)]" href="#contact" onClick={e => { e.preventDefault(); setMobileOpen(false); const el = document.getElementById('contact'); if (el) { const y = el.getBoundingClientRect().top + window.scrollY - 64; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Contact</a>
                </nav>
            </div>
        </>
    )
